...
import os, io, csv, re, uuid, math, time
...
def extract_upload(name: str, data: bytes) -> str:
    ...
    if suffix == ".csv":
        rows = csv.reader(
            io.StringIO(data.decode("utf-8", errors="ignore"))
        )
    ...
    raise HTTPException(
        400,
        "Supported formats are PDF, TXT, DOCX, and CSV"
    )
...
async def root():
    return {
        "message": "RAG Document Intelligence API is ready"
    }
...
async def download_source_zip():
    archive = Path(
        "/app/rag-document-intelligence-assistant.zip"
    )

    if not archive.exists():
        raise HTTPException(
            404,
            "Source archive is not available"
        )

    return FileResponse(
        archive,
        media_type="application/zip",
        filename="rag-document-intelligence-assistant.zip"
    )
...
async def upload_documents(
    ...
    print(
        f"[UPLOAD] Processing time: {upload_time:.2f}s"
    )
...
async def ask(request: AskRequest):
    ...
    answer = await gemini(
        ...
        f"Context:\n{context}\n\n"
        f"Answer with a concise explanation "
        f"and inline source citations.",
        f"{request.session_id}-answer-{uuid.uuid4()}"
    )
    ...
    retrieval = round(
        sum(r["score"] for r in retrieved)
        / max(1, len(retrieved))
        * 100
    )
...
async def evaluate(request: EvalRequest):
    ...
    if not session["chunks"]:
        raise HTTPException(
            400,
            "Upload documents before evaluating"
        )
    ...
    for question in request.questions[:8]:
        ...
        answer = await gemini(
            "Evaluate this question using "
            "the context. Return a short "
            "answer only.\n"
            f"Question: {question}\n"
            f"Context: {' '.join(c['text'] for c in ranked)}",
            f"{request.session_id}-eval-{uuid.uuid4()}"
        )
